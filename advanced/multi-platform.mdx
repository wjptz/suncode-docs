...
Legend: ✅ Trellis wires the config and the platform executes it · ⚡ Partial (Codex requires `codex_hooks = true` opt-in; Kiro's Agent Hooks are user-configured, not auto-executed) · ❌ the platform does not expose this event · — the platform has no command primitive; start / finish-work / continue are delivered as skills instead. Qoder uses `/trellis-{name}` (hyphen, not colon) via its native Custom Commands for `finish-work` / `continue`; `start` is covered by its SessionStart hook.

...
Skills ship on all 22 configured platforms — the location differs per platform:
...
| Codex       | `.agents/skills/{name}/SKILL.md` (plus Codex-specific overrides under `.codex/skills/`) |
...
| Pi Agent    | `.agents/skills/{name}/SKILL.md`                                                        |
...
| Kimi Code   | `.agents/skills/{name}/SKILL.md` (private command/role skills use `.kimi-code/skills/`) |
| Snow CLI    | `.snow/skills/{name}/SKILL.md`                                                          |

`.agents/skills/{name}/SKILL.md` (the [agentskills.io](https://agentskills.io) cross-platform shared layer) is written with neutral placeholders by Codex, Gemini CLI, Pi Agent, Kimi Code, and ZCode. Their output must be byte-identical and is directly usable by Amp, Cline, Deep Agents, Firebender, Warp, and other agents that read the standard.

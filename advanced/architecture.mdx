...
If the platform doesn't expose a session identity, `task.py start` exits with a hint to set `TRELLIS_CONTEXT_ID` explicitly — current code does not write a global `.trellis/.current-task` fallback. (Older projects may still have a leftover `.trellis/.current-task` file; it is preserved but ignored by active-task resolution.)

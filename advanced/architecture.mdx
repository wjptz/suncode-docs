...
│  Qoder / CodeBuddy / Copilot / Droid / Pi / Kilo /           │
│  Antigravity / Windsurf + .agents/skills ecosystem           │
...
- **`/trellis:continue`** reads the current session runtime (`.trellis/.runtime/sessions/<session-key>.json`) + task `status`, then consults `workflow.md` to figure out which step the task is on and what the next action should be.
...
<Tabs>
  <Tab title="Hook / extension capable (CC / Cursor / OC / Gemini / Qoder / CodeBuddy / Copilot / Droid / Pi)">
    SessionStart hook or extension fires automatically, so the platform-specific `session-start.py` / `session-start.js` / Pi extension injects Trellis context at every session open with no manual step. Describe your task directly; optionally run `/trellis:start` on platforms that expose it for an explicit context report.
  </Tab>
  <Tab title="Codex">
...
<Info>
  Automatic sub-agent spec injection relies on the platform's PreToolUse hook or extension equivalent. Claude Code, Cursor, OpenCode, CodeBuddy, Droid, and Pi Agent support this today. On the other platforms, the main session reads the JSONL files itself and passes the relevant content into sub-agent prompts.
</Info>

`inject-subagent-context` (Python on Claude Code, JS on OpenCode, extension code on Pi) is the core spec-injection engine. When the main session spawns a `trellis-*` sub-agent, the hook or extension intercepts the call and injects the right JSONL-specified context.
...
│  (runs BEFORE sub-agent)                                     │
│                                                              │
│  The hook / extension (not the sub-agent) does the reading:  │
│                                                              │
│  1. It reads the current session runtime                     │
│     → .trellis/.runtime/sessions/<session-key>.json          │
│     → current task directory path                            │
│                                                              │
│  2. It reads {task_dir}/implement.jsonl                      │
│     → list of spec files this sub-agent needs                │
│                                                              │
│  3. It reads each file referenced in the JSONL               │
│     → .trellis/spec/backend/index.md                         │
│     → .trellis/spec/backend/database-guidelines.md           │
│     → ...                                                    │
│                                                              │
│  4. It reads prd.md (requirements) and info.md (design)      │
│                                                              │
│  5. It concatenates everything and prepends it to the        │
│     sub-agent's prompt as "additionalContext"                │
└───────────────────────┬──────────────────────────────────────┘
                        │
                        ▼
The sub-agent wakes up with this already in its prompt on hook-push and extension-backed platforms (it does not read any of the files itself). Pull-based platforms put equivalent read instructions in the sub-agent prelude:

...
<Note>
  Sub-agents ship on 11 of 16 configured platforms: Claude Code, Cursor, OpenCode, Codex, Kiro,
  ...
  present via a pull-based prelude instead of having that context injected. Codex can also run in
  inline mode, where the main session reads the same artifacts through skills. Kilo, Antigravity,
  and Devin do not expose a sub-agent primitive — the implement / check work runs inline in the main
  session.
</Note>

...
A spec template marketplace is a Git-backed source that `suncode init --registry`
can read when a team wants the same `.suncode/spec/` starting point across many
repositories.
...
`suncode init --registry <source>` parses `<source>` as a GitHub, GitLab, or
Bitbucket source, then probes for `index.json` inside that source path.

| Mode             | Condition                                     | Behavior                                                                |
| ---------------- | --------------------------------------------- | ----------------------------------------------------------------------- |
| Marketplace mode | `<source>/index.json` exists and is valid     | Suncode lists `type: "spec"` templates and installs the selected `id`   |
| Direct mode      | `<source>/index.json` returns not found       | Suncode downloads the source directory itself into `.suncode/spec/`     |
| Error            | `index.json` exists but is invalid/unreadable | Suncode stops instead of guessing direct mode from a broken marketplace |
...
`.suncode/spec/`. Do not wrap them in an extra `spec/` folder unless you really
want `.suncode/spec/spec/...` after install.

The inner folder names are your convention. Suncode does not require
...
`--registry` at `gh:myorg/my-spec-marketplace/marketplace`, so Suncode reads:
...
| `path`        | Yes      | Directory copied into `.suncode/spec/`, relative to repository root.   |
...
After install, the target project owns its `.suncode/spec/` files. The template
...
- `.suncode/tasks/`, `.suncode/workspace/`, or active task state.
- Platform prompt files such as `.claude/`, `.codex/`, `.cursor/`, `.opencode/`,
  or `.engineer/`. Those belong in Suncode platform customization, not spec
  templates.
...
| Direct mode runs instead of marketplace mode | Suncode did not find `<source>/index.json`. Point `--registry` at the marketplace folder.  |
| Files land under `.suncode/spec/spec/`       | The template directory contains an extra outer `spec/` folder.                             |

...
Matches `task.py create` in `.suncode/scripts/common/task_store.py`:
...
```json
{
  ...
  } // Creation-time execution policy snapshot; other namespaces such as meta.hub may coexist
}
```

Field order above matches `.suncode/scripts/common/task_store.py` verbatim — `task.py` writes them in this order.

<Note>
  `worktree_path` / `commit` / `pr_url` are schema placeholders — written as null on create, and no
  Suncode script updates them afterward. Store commit hashes / PR URLs under `meta: {}` as custom
...
`meta.execution` is written for new tasks from the current
`execution.dag.enabled` and `execution.dag.require_for_complex_tasks` settings.
The snapshot prevents later configuration changes from retroactively enforcing
new planning gates on existing tasks. Older tasks without this namespace use
the compatibility path. `meta` remains extensible; Hub binding data, for
example, lives under `meta.hub` beside `meta.execution`.

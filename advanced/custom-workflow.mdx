...
`/.suncode/workflow.md` defines how Suncode runs development. Phase definitions, skill routing, per-turn reminders, and the `task.py` command catalog all live in this one file. Forking the workflow means editing one markdown file — no Python, no hook code, no re-releasing Suncode.
...
| `### Skill Routing`                                     | AI at session start                                    | Maps user intent → which skill to load (e.g. "wants new feature" → `suncode-brainstorm`) |
...
You can change phase wording and routing, but execution planning has a platform-neutral runtime
contract. A task's plan is materialized once as `.suncode/tasks/<task>/execution.json`; inline,
native sub-agent, and channel modes all consume that same DAG.

When customizing Plan or Execute, preserve these invariants:

- express ordering with explicit `dependsOn` edges, not list order or tree position;
- give independently runnable work separate nodes, with honest read/write/resource declarations;
- keep one final barrier reachable from every branch;
- dispatch every selected conflict-free ready node before waiting, then resume when any node
  completes and recompute readiness immediately;
- treat the main session as coordinator while delegated work is active;
- deliver each delegated node through its generated context manifest and bounded content rather
  than forwarding the parent conversation transcript;
- keep check nodes read-only and route repairs through a single write-owning fix node.

Inline execution is the compatibility path, not a second workflow: it lowers effective
concurrency to `1` while preserving the same nodes, dependencies, context, and results. Legacy
tasks without `execution.json` are normalized conservatively; a custom workflow should not create
different inline and sub-agent plans.
...
surface the blocker to the user or spawn `suncode-research` if more investigation helps.
[/workflow-state:blocked]
```
...
| User intent                               | Skill               |
| ----------------------------------------- | ------------------- |
| Wants a new feature / requirement unclear | suncode-brainstorm  |
| About to write code / start implementing  | suncode-before-dev  |
| Finished writing / want to verify         | suncode-check       |
| Stuck / fixed same bug several times      | suncode-break-loop  |
| Spec needs update                         | suncode-update-spec |
```

Add a row for a custom skill you shipped (see [Custom Skills](./custom-skills)):

```markdown
| User wants a test plan before writing code | suncode-test-plan |
```
...
| The single-DAG and fan-out-before-wait execution invariants       | All executor modes share `execution.json` and its persisted run state      |

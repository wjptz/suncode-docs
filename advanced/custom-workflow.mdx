...
| ------------------------------------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `## Phase Index` + `## Phase 1/2/3`                     | AI startup path, `trellis-start`, and `/trellis:continue` | Defines the three-phase flow and the step-by-step how-to inside each phase               |
| `### Skill Routing`                                     | AI startup path and skills                                | Maps user intent → which skill to load (e.g. "wants new feature" → `trellis-brainstorm`) |
...
- Keep each block compact. This is injected every turn; bloat means the AI pays attention cost on every message.
...
Keep the Phase Index in sync with the detailed Phase sections. Startup context
usually injects the compact Phase Index, while `/trellis:continue` and
`get_context.py --mode phase --step X.Y` load detailed step bodies on demand.

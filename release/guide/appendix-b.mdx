...
Every command below is available on **all 12 supported platforms** in 0.4.0 GA. The invocation syntax varies per platform (see the next subsection); semantics are identical.
...
That's 13 commands total. Claude Code / OpenCode / iFlow / Factory Droid ship all 13. Other platforms ship 12 (no `parallel`).

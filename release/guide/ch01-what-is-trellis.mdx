...
| **Monorepo chaos** | One generic rules file can't cover multiple packages with different conventions | `trellis init` auto-detects monorepo layout and creates **per-package** spec directories — each package enforces its own standards |
...
| Platform support | Cursor only | Claude Code only | Per platform | **12 platforms** (Claude Code, Cursor, OpenCode, Codex, iFlow, Gemini CLI, Kilo, Kiro, GitHub Copilot, Windsurf, Qoder, Factory Droid) |

...
The core systems (specs, tasks, workspace) work. You read spec files and follow them.

What doesn't work: hooks. Cursor can't do the automatic context injection that Claude Code has. You'll need to manually reference spec files at session start.

Multi-agent workflows also don't work in Cursor.

</Accordion>

<Accordion title="Do I need to use all three systems?">
No. Most people start with specs only. Write your conventions, let AI follow them.

Add tasks when you need context isolation between work items. Add workspace when you want session history that persists.
...
For task-specific work, run `trellis task start` first.
...
They work in separate Git worktrees. No conflicts during development.

Merge conflicts happen when merging PRs. Resolve normally. To avoid: plan tasks so they don't overlap.
...
Specs might not be loading. Check:

1. Spec file is in `.trellis/spec/`?
2. Paths correct in JSONL context files?
3. For task work, is the task set as current?

Also: is your spec specific enough? Vague specs get ignored.
...
Then update your project. Two options:
...
Only updates files that haven't been modified. Your customizations are preserved.
...
Overwrites all Trellis files with latest templates. Use this when you want a fresh start or the new version has breaking changes. Back up your customizations first.

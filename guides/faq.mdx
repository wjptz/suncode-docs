...
For task-specific work, run `task.py start "$TASK_DIR"` in the current AI session/window, or use `/trellis:continue` after the task is created and planned. Trellis stores that active task under `.trellis/.runtime/sessions/<session-key>.json`.

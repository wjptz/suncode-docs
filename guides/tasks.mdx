...
```bash
python .trellis/scripts/task.py create "user-auth" --assignee taosu
```
...
```bash
python .trellis/scripts/task.py set 01-31-user-auth-taosu
```
...
```bash
python .trellis/scripts/task.py status 01-31-user-auth-taosu done
```
...
```bash
python .trellis/scripts/task.py set other-task-id
```
...
4. Set as current: `task.py set`
5. Work in Claude Code sessions
6. Mark complete: `task.py status <id> done`

...
| **Python** | 3.9 | Hooks and automation scripts (matches macOS system `python3`; Ubuntu 22.04 LTS and newer are fine) |
| **git** | any recent | Task branching, worktrees, and commit metadata |

`trellis init` soft-warns if `python3 --version` is older than 3.9 but won't block — if you see the warning, upgrade Python before continuing.

...
Claude Code statusLine is no longer installed by default. New installs do not write `.claude/hooks/statusline.py` or configure `statusLine` in `.claude/settings.json`.

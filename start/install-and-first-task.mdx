...
suncode init -u your-name --claude --cursor --opencode
...
Beyond these 16 configured platforms, any AI coding agent that reads the `.agents/skills/` standard (Amp, Cline, Deep Agents, Firebender, Kimi Code CLI, Warp, and more) can also consume Suncode: Codex writes its skills there, and the files are directly usable by the rest of that ecosystem.
...
| **Gemini CLI**  | `.gemini/commands/suncode/`, `agents/`, `skills/`, `hooks/`                  |
...
`.agents/skills/` is a shared cross-platform layer ([agentskills.io](https://agentskills.io) standard). `suncode init` writes all skills into it too, so any agent that reads the `.agents/skills/` standard (Amp, Cline, Deep Agents, Firebender, Kimi Code CLI, Warp, etc.) can consume them.
...
<Note>
  ...
  `suncode-start`. Only agent-less platforms (Kilo, Antigravity, Devin) ship `/suncode:start` (or

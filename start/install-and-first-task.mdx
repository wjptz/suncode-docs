...
**When AI stalls or is skipping ahead**: run `/trellis:continue`. AI reads the current session runtime (`.trellis/.runtime/sessions/<session-key>.json`) + the task's status and, per `workflow.md`, figures out the next step (before-dev / check / update-spec, etc.) and continues.
...
Journals live under `.trellis/workspace/{name}/journal-N.md`; every `/trellis:finish-work` appends one. `.trellis/.runtime/sessions/<session-key>.json` points this AI session/window to its unfinished task, if any, and `task.py finish` clears it.

...
description: 'Use Suncode in real engineering work: new products, brownfield repos, refactors, bug fixes, and team rollouts.'
icon: 'map'
---
...
Each scenario below gives a starting prompt, the Suncode files to produce, the workflow to follow, and a concrete finish line. Pick the closest situation and adapt it into a task for your repo.

<Note>
  Suncode is skill-first. Treat the prompts below as task inputs; Suncode routes the work through
  the relevant skills for brainstorming, spec loading, checks, and knowledge capture. Use
  `/suncode:start` only when your platform needs a manual session entry point.
</Note>

## Scenario map

| Scenario                                                                 | Use when                                             | Main Suncode value                                    |
| ------------------------------------------------------------------------ | ---------------------------------------------------- | ----------------------------------------------------- |
...
1. Run `suncode init` once the project is initialized — it writes around 17 default spec templates under `.suncode/spec/`, split into `backend/` / `frontend/` / `guides/` (directory structure, error handling, logging, component and hook guidelines, cross-layer thinking, etc.), and auto-creates a `00-bootstrap-guidelines` task.
...
3. Optional: use the bundled `suncode-spec-bootstrap` skill to draft first-pass specs from the real codebase, plus any other skills that match the stack. Suncode installs this bundled skill automatically.
4. Fill in the default spec templates produced by `suncode init`, focusing on what the current work needs; do not pre-design the whole project.
5. Review the generated specs by hand for quality.
6. Create one task for the smallest useful vertical slice.
7. Type `/suncode:continue` repeatedly to drive the task to completion — Suncode routes through check / update-spec / finish per `workflow.md`.
8. Run `/suncode:finish-work` to archive the task and record the session journal.
...
Inspect the codebase and propose a minimal Suncode bootstrap for the next feature task. Identify the
...
1. Run `suncode init` — it writes the default spec templates and auto-creates a `00-bootstrap-guidelines` task that drives the rest of this scenario.
...
3. Optional: use the bundled `suncode-spec-bootstrap` skill to draft first-pass specs from the real codebase, then review the output by hand. Suncode installs this bundled skill automatically.
...
Drawn from the `suncode-update-spec` skill's writing principles:
...
- Reviewers can point to `.suncode/spec/` instead of re-explaining conventions.
...
```text
Create a Suncode task for team invitations.
...
The Suncode task is the feature boundary: one PRD, one implementation context,
...
Task files: .suncode/tasks/<date>-team-invitations/prd.md, implement.jsonl, check.jsonl
```
...
| Cross-layer check        | End-to-end path from create invite to accepted membership               | `suncode-check` verifies UI input, API validation, database writes, permissions, email |

Keep these subtasks under the same Suncode task when they must ship together.
Split a separate Suncode task only when the work has its own scope and release
boundary, such as bulk CSV import, SSO provisioning, or role templates.

### 3.5 Workflow

1. **User** describes the feature in natural language; answers clarifying questions and confirms scope as planning progresses → **AI** classifies the request and asks for Suncode task creation. After the user agrees, AI loads `suncode-brainstorm`, inspects relevant code/docs before asking repo-answerable questions, creates the task (`task.py create`), and captures clarified Goal / In scope / Out of scope / Acceptance criteria into `prd.md` (Phase 1.1).
2. **AI** configures `implement.jsonl` / `check.jsonl` with the specs the task touches (Phase 1.3).
3. **User** types `/suncode:continue` → **AI** dispatches `suncode-implement` and builds the smallest end-to-end slice against the PRD (Phase 2.1).
4. **User** types `/suncode:continue` → **AI** dispatches `suncode-check`, reviewing cross-layer contracts via `check.jsonl` (UI input, API validation, database writes, email side effects, permission-failure paths) and fixing issues in place (Phase 2.2).
5. **User** types `/suncode:continue` → **AI** advances to Phase 3, checks whether the task produced any new reusable specs, and triggers `suncode-update-spec` to update `.suncode/spec/` if it did.
6. **User** types `/suncode:finish-work` → **AI** archives the task and records the session journal.

## 4. Refactor a legacy module

Use this when code works but is hard to change. A Suncode refactor task should be behavior-preserving by default.
...
5. **User** types `/suncode:continue` → **AI** `suncode-implement` extracts one responsibility per round; public interfaces stay stable by default (Phase 2.1).
6. **User** types `/suncode:continue` → **AI** `suncode-check` runs the tests; failures trigger Phase 2.3 rollback of the current extraction.
7. Repeat 5-6 once per responsibility; user reviews each round's diff.
8. **User** types `/suncode:continue` → **AI** triggers `suncode-update-spec` to record the new module boundaries → **User** confirms which entries are long-term rules.
9. **User** types `/suncode:finish-work` → **AI** archives the task and records the session journal.
...
3. **User** types `/suncode:continue` → **AI** `suncode-implement` ships the smallest safe fix and adds the regression test before any broader cleanup (Phase 2.1).
4. **User** types `/suncode:continue` and hand-reviews whether the patch actually addresses the reported behavior → **AI** `suncode-check` runs the tests; the regression test must pass with the patch and fail without it (Phase 2.2).
5. **User** types `/suncode:continue` → **AI** runs `suncode-break-loop` for root-cause analysis (Phase 3.2) → **User** confirms whether the conclusion really prevents recurrence.
6. **User** types `/suncode:continue` → **AI** routes the prevention into a spec, test helper, or checklist via `suncode-update-spec` → **User** confirms whether each item is a long-term rule (Phase 3.3).
7. **User** types `/suncode:finish-work` → **AI** archives the task and records the session journal.
...
```text
Review the last few PR comments and help me convert repeated engineering feedback into Suncode specs.
...
3. **User** types `/suncode:continue` → **AI** `suncode-update-spec` adds each short rule to the most relevant spec file with a good/bad code example → **User** reviews wording for accuracy.
4. **AI** wires the updated specs into the next development task's `check.jsonl` so `suncode-check` can verify the rules catch issues.
5. **User** runs a real task or two and observes which rules failed to catch issues or created noise → **AI** removes or rewrites those rules via `suncode-update-spec`.
6. **User** types `/suncode:finish-work` → **AI** archives the spec-tightening task and records the session journal.

## 7. Roll out to a team

Use this when Suncode adoption involves multiple developers, repos, or AI tools.
...
```text
Create a Suncode rollout plan for one pilot repo and one department.
...
| ----- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | Pilot one repo            | One real task completed with specs, checks, and journal. Two options for the spec starting point: pull a stack-matched spec pack from the [spec template marketplace](/start/install-and-first-task#official-marketplace), or use `suncode-spec-bootstrap` so AI drafts first-pass specs from the real codebase |
| 2     | Capture repeated feedback | Three to five review patterns become specs                                                                                                                                                                                                                                                                      |
| 3     | Standardize task workflow | Developers know when to describe work directly, when `/suncode:continue` advances an active task, and when `/suncode:finish-work` archives after commits                                                                                                                                                        |
| 4     | Add platform adapters     | Multiple AI tools consume the same `.suncode/` context                                                                                                                                                                                                                                                          |
...
<CardGroup cols={2}>
  ...
  <Card title="Team rollout" icon="users" href="#7-roll-out-to-a-team">
    Pilot Suncode in one repo before expanding usage.
  </Card>
</CardGroup>

...
1. Run `trellis init` once the project is initialized — it writes default spec templates under `.trellis/spec/` and auto-creates a `00-bootstrap-guidelines` task.
...
5. Edit those specs manually. Remove guesses, vague rules, and future plans.
6. Create one task for the smallest useful vertical slice.
7. Build that slice with check and finish steps enabled.
8. Record the session so stack decisions and tradeoffs are easy to recover.

```bash
trellis init -u alice

# Agent-capable platforms (Claude Code / Cursor / OpenCode / etc.): open an AI session and type the prompt below.
# Agent-less platforms (Kilo / Antigravity / Windsurf): run /trellis:start first.
...
```bash
# Agent-capable platforms: type the prompt directly. Agent-less platforms: run /trellis:start first.

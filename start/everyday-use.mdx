...
`continue` is a **within-task** continue — not a cross-task one. It reads `.trellis/.current-task` and the task's `status`, consults `workflow.md` to locate the current phase/step, and advances to the next one.

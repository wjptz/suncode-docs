...
Run this at the beginning of a session if your platform does not auto-inject
context. On hook-capable or extension-capable platforms, a SessionStart hook,
extension, or prompt hook handles orientation, so `start` is usually not
installed as a user-facing command. Codex uses `AGENTS.md` plus a
`UserPromptSubmit` breadcrumb; when no task is active, that breadcrumb may tell
...
Prerequisite: code is already committed. The AI drives a batched commit step in
workflow Phase 3.4 (see `.trellis/workflow.md`) where it drafts commits from this
session's edits, learns the repo's commit-message style from `git log --oneline
-5`, presents the plan once for one-shot user confirmation, and runs
`git commit` per batch. `/finish-work` itself focuses on archive + journal and
stops if current-task work is still uncommitted.
...
2. `git status --porcelain`, classifying current-task work separately from unrelated dirty paths and Trellis bookkeeping paths.
...
Sub-agents are isolated AI sub-processes with their own prompt and
(platform-specific) their own tool / hook wiring. Implement/check agents receive
spec and research context through JSONL files, then read task artifacts in the
shared order: `prd.md`, `design.md` if present, `implement.md` if present.
...
On Claude Code, Cursor, OpenCode, CodeBuddy, Droid, and Pi Agent, implement/check
sub-agents get the right JSONL entries and task artifacts injected automatically
before they start. Pi uses its extension rather than a Python hook. On the rest,
sub-agents use a pull-based prelude or the main session reads the artifacts
inline, depending on platform and mode.
...
```
classify → create → plan artifacts → optional jsonl → start → implement/check → finish → archive

classify:       AI asks for task-creation consent when needed
create:         task directory + task.json + default prd.md
plan artifacts: AI writes prd.md; complex tasks also get design.md + implement.md
optional jsonl: AI fills implement/check spec/research manifests when useful
start:          writes current task for this AI session/window and status=in_progress
...
`task.py create` always creates the default `prd.md` template. It may also seed
`implement.jsonl` + `check.jsonl` when a sub-agent-capable platform is installed
(Claude / Cursor / Codex / Kiro / Pi / etc.). It does not create `design.md` or
`implement.md`; those are written during planning for complex tasks.
...
`task.py add-context` only writes to `implement.jsonl` / `check.jsonl`. Research
findings belong under `$TASK_DIR/research/` as durable markdown files.

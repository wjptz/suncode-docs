---
title: 'open-typeless'
description: 'Step-by-step guide: Building a macOS voice input app with Trellis'
---

# open-typeless

A step-by-step tutorial showing how to use Trellis to build a macOS voice input app from scratch.

<Info>
  **Source Code**:
  [github.com/mindfold-ai/open-typeless](https://github.com/mindfold-ai/open-typeless)
</Info>

## Step 1: Project Initialization

### 1.1 Create Electron Project

```bash
npx create-electron-app@latest open-typeless --template=vite-typescript

cd open-typeless

# Remove npm generated files
rm -rf node_modules package-lock.json

# Create .npmrc (required for pnpm + Electron)
cat > .npmrc << 'EOF'
node-linker=hoisted
shamefully-hoist=true
EOF

# Reinstall with pnpm
pnpm install
```

### 1.2 Initialize Trellis

```bash
trellis init
```

![trellis init](/images/use-cases/open-typeless/init.png)

### 1.3 Copy Specs from Existing Project

If you have specs from a similar project, copy them over:

```bash
cp -r ../old-project/electron-doc ./
```

### 1.4 Ask AI to Fill in Specs

**Prompt:**

> Help me select useful specs from electron-doc/ and organize them into this project's .trellis/spec/

AI will analyze and organize specs:

![spec selection](/images/use-cases/open-typeless/step1.png)

## Step 2: Task Planning

### 2.1 Ask AI to Plan Tasks

**Prompt:**

> I want to use Volcengine ASR BigModel API to build this. Help me plan how to break down the tasks.

AI creates a batch-based task plan:

![task planning](/images/use-cases/open-typeless/step2-1.png)

### 2.2 Create Tasks

AI creates tasks organized into batches:

| Batch   | Tasks                                                                | Purpose                          |
| ------- | -------------------------------------------------------------------- | -------------------------------- |
| Batch 1 | `asr-infrastructure`                                                 | Foundation (must complete first) |
| Batch 2 | `asr-audio-recorder`, `asr-volcengine-client`, `asr-floating-window` | Can run in parallel              |
| Batch 3 | `asr-integration`                                                    | Integration (depends on Batch 2) |

![tasks created](/images/use-cases/open-typeless/step2-2.png)

### 2.3 Complete Batch 1

After Batch 1 completes, verify and update downstream task contexts:

![batch 1 complete](/images/use-cases/open-typeless/step2-3.png)

## Step 3: Parallel Development

### 3.1 Start Parallel Agents

Use `/trellis:parallel` to launch 3 worktree agents for Batch 2:

```bash
./.trellis/scripts/multi-agent/start.sh .trellis/tasks/01-29-asr-audio-recorder
./.trellis/scripts/multi-agent/start.sh .trellis/tasks/01-29-asr-volcengine-client
./.trellis/scripts/multi-agent/start.sh .trellis/tasks/01-29-asr-floating-window
```

![parallel agents](/images/use-cases/open-typeless/step3-1.png)

Each agent works in an isolated git worktree, developing features independently.

## Step 4: Monitor Progress

### 4.1 Check Agent Status

```bash
./.trellis/scripts/multi-agent/status.sh
```

![agent status](/images/use-cases/open-typeless/step4-1.png)

Shows: 1 running / 4 registered agents, task progress by priority.

### 4.2 Record Session

After parallel agents complete, they create PRs automatically:

![parallel PRs](/images/use-cases/open-typeless/step3-4.png)

After merging and completing a batch, record the session:

**Prompt:** `/trellis:record-session`

![record session](/images/use-cases/open-typeless/step2-4.png)

Then archive completed tasks:

```bash
./.trellis/scripts/task.sh archive asr-infrastructure
```

## Step 5: Continue Development

### 5.1 Check Remaining Tasks

```bash
./.trellis/scripts/task.sh list
```

![task list](/images/use-cases/open-typeless/step5-1.png)

### 5.2 Implement Next Feature

Start the next task and let AI implement:

```bash
./.trellis/scripts/task.sh start .trellis/tasks/01-29-push-to-talk
```

The hook automatically injects task context. AI uses implement agent then check agent:

![implement and check](/images/use-cases/open-typeless/step5-2.png)

### 5.3 Configure and Test

AI helps with remaining setup (environment config, permissions):

![final setup](/images/use-cases/open-typeless/step5-3.png)

## Summary

Using Trellis to build open-typeless:

| Step | What                 | Trellis Feature                        |
| ---- | -------------------- | -------------------------------------- |
| 1    | Initialize project   | `trellis init`, spec organization      |
| 2    | Plan tasks           | AI task breakdown, batch planning      |
| 3    | Parallel development | `/trellis:parallel`, worktree agents   |
| 4    | Monitor & record     | `status.sh`, `/trellis:record-session` |
| 5    | Continue iterating   | Task hooks, implement/check agents     |

**Result:** Complete Electron app in 1 day, with structured specs and documented progress.

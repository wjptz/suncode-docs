...
- `status` → transitions `planning → in_progress → completed` (also `rejected` from the brainstorm skill)
- `completedAt` / `commit` → set when the task is archived
- `pr_url` → set when a PR is opened against the task's branch
- `parent` / `children` → set via `task.py create --parent` / `add-subtask`

Older tasks created before a field existed may be missing some keys (e.g. tasks created pre-`package` support won't have `"package"`); `task.py` treats missing fields as null, so nothing breaks. Legacy `current_phase` / `next_action` from earlier Multi-Agent Pipeline versions are still read if present on old tasks, but `task.py create` no longer writes them.
...
task.py archive    →  completed (then moved to archive/)
```

`planning` / `in_progress` / `completed` are the default set — they line up with the three phases in `workflow.md`. `task.py --status` also accepts `review` as a filter for custom workflows that introduce it; add any additional statuses you need by writing a matching `[workflow-state:<name>]` block in `workflow.md`.

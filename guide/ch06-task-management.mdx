...
<Note>
  The **bootstrap** task (auto-created by `trellis init`) goes through a separate schema in `packages/cli/src/commands/init.ts` — slightly different field set (no `title` / `scope` / `package` / `branch` / `base_branch` / `worktree_path` / `pr_url`) and `status` is initialized to `in_progress`. The breaking-update **migration task** (`trellis update` breaking flow) uses yet another schema with additional `current_phase` / `next_action` fields — those two fields appear only on migration tasks.
</Note>

---
title: 'Appendix D: JSONL Configuration Format Reference'
---

## Appendix D: JSONL Configuration Format Reference

### File entry

```jsonl
{"file": ".trellis/spec/backend/index.md", "reason": "Backend development guide"}
```

### Directory entry

```jsonl
{"file": "src/services/", "type": "directory", "reason": "Existing service patterns"}
```

For directory entries, the hook reads all `.md` files in the directory, up to 20.

### Complete example (fullstack `implement.jsonl`)

```jsonl
{"file": ".trellis/workflow.md", "reason": "Project workflow and conventions"}
{"file": ".trellis/spec/shared/index.md", "reason": "Shared coding standards"}
{"file": ".trellis/spec/backend/index.md", "reason": "Backend development guide"}
{"file": ".trellis/spec/backend/api-module.md", "reason": "API module conventions"}
{"file": ".trellis/spec/backend/quality.md", "reason": "Code quality requirements"}
{"file": ".trellis/spec/frontend/index.md", "reason": "Frontend development guide"}
{"file": ".trellis/spec/frontend/components.md", "reason": "Component conventions"}
```

### JSONL files used by each sub-agent

| File              | Sub-agent           | Typical content                                   |
| ----------------- | ------------------- | ------------------------------------------------- |
| `implement.jsonl` | `trellis-implement` | workflow + specs + code pattern examples          |
| `check.jsonl`     | `trellis-check`     | quality checklist specs + lint / typecheck rules  |
| `research.jsonl`  | `trellis-research`  | project structure overview + investigation targets|

### Removed in 0.5.0-beta.0

Earlier versions also referenced `debug.jsonl` for a separate `debug` sub-agent. Both the file and the sub-agent were removed; bug fixing now happens in the main session or through `trellis-break-loop`.

The old **completion marker** scheme (generating strings like `TYPECHECK_FINISH` from `reason` fields so the Ralph Loop could detect when the check agent was "done") is also gone. The `trellis-check` sub-agent's own retry loop no longer relies on markers.

---

...
<Note>
Codex hooks require enabling the experimental feature flag. Add this to `~/.codex/config.toml`:
...
[CodeBuddy](https://copilot.tencent.com/) is Tencent Cloud's AI coding assistant, supporting IDE and CLI forms with MCP extensibility.

```bash
trellis init --codebuddy -u your-name
```

Generates configuration under the `.codebuddy/` directory:

**Trellis to CodeBuddy mapping**:

| Trellis / Claude Code | CodeBuddy | Description |
|----------------------|-----------|------|
| `CLAUDE.md` | `.codebuddy/.rules/*.md` | CodeBuddy uses rules files for project context |
| `.claude/commands/*.md` | `.codebuddy/commands/trellis/*.md` | Nested directories: `/trellis:start`, `/trellis:check` |
| `.claude/agents/*.md` | `.codebuddy/agents/*.md` | Custom AI sub-agents (markdown + YAML frontmatter) |
| `.claude/hooks/` | `settings.json` hooks | CodeBuddy supports hooks in settings config |

**CodeBuddy configuration directory structure**:

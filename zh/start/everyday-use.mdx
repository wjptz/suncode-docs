...
0.5.0 开始 Trellis 走 **skill-first**：大部分能力都做成 auto-trigger skill，平台自己根据上下文触发，不用你记命令。显式 slash 命令只留了三个，用来处理跨会话切换的节点。
...
| **Command**    | `/trellis:continue`        | 手动                                                   | 当前任务内推进下一步工作流，用户不用记 workflow（详见 1.2.3 小节） |
...
#### 1.2.2 `/trellis:finish-work`：收尾 + 归档
...
#### 1.2.3 `/trellis:continue`：当前任务内推进下一步

`continue` 是**单个任务内**的 continue，不是跨任务的 continue。它读 `.trellis/.current-task` 和该任务的 `status`，对照 `workflow.md` 判断当前处于哪个 phase/step，然后推进到下一步。
...
create → curate jsonl → start → implement/check → finish → archive
  ...
  │           │            │          │               │         │
  ▼           ▼            ▼          ▼               ▼         ▼
创建        AI 填 seed    设为当前   开发 /           清除      归档到
目录 +      过的 jsonl    任务指针   检查循环         当前      archive/
task.json   （或 add-                                任务
+ seed      context）
jsonl
```

`task.py create` 在检测到 sub-agent 平台（Claude / Cursor / Codex / Kiro 等）时会顺手 seed 出 `implement.jsonl` + `check.jsonl`；agent-less 平台（Kilo / Antigravity / Windsurf）跳过 seed，改由 Phase 2 的 `trellis-before-dev` skill 加载 spec。
...
`trellis-update-spec` skill 把 spec 当作**可执行契约**写，不是原则性文字。sub-agent 在 `trellis-implement` / `trellis-check` 时读到的每条都要能告诉它 *怎样安全地实现*——具体签名、契约、用例、测试。如果你写的只是"动手前该想到什么"，那东西属于 `guides/`。
...
`trellis init` 会同时建一个 bootstrap 任务（`00-bootstrap-guidelines`）。第一次 `/trellis:start` 时 AI 会认出它，调 `trellis-research` 扫一遍你的代码，然后把 `frontend/` / `backend/` / `guides/` 下那堆空模板按你的真实项目填起来——技术栈、约定、目录结构都从代码里读出来。

...
如果平台不自动注入上下文就手动跑一下。Hook 或 extension 可用的平台通过
SessionStart hook、extension 或 prompt hook 完成 orient，所以通常不会安装
用户可见的 `start` 命令。Codex 使用 `AGENTS.md` 加 `UserPromptSubmit`
breadcrumb；没有 active task 时，这段 breadcrumb 可能会提示 AI 读取一次
`trellis-start`。
...
前提：代码已经 commit。AI 在 workflow Phase 3.4（见 `.trellis/workflow.md`）里主导批量 commit —— 起草本次 session 改动的 commit 计划、参考 `git log --oneline -5` 学习 repo 的 commit message 风格、一次性展示计划让用户 confirm、确认后逐批 `git commit`。`/finish-work` 自身只做 archive + journal；如果当前任务改动仍未提交，它会停止，保证 bookkeeping commit 在 work commit 之后。
...
2. 跑 `git status --porcelain`，区分当前任务改动、无关脏文件和 Trellis bookkeeping 路径。
...
Sub-agent 是独立的 AI 子进程，各自有 prompt 和（视平台而定）各自的 tool / hook 挂接。Implement/check agent 通过 JSONL 拿 spec 和 research 上下文，然后按统一顺序读取 task artifact：`prd.md`、存在时的 `design.md`、存在时的 `implement.md`。
...
在 Claude Code、Cursor、OpenCode、CodeBuddy、Droid、Pi Agent 上，implement/check
sub-agent 启动前会自动拿到对应 JSONL 条目和 task artifact。Pi 走 extension，
不是 Python hook。其他平台根据自身模式使用 pull-based prelude，或由主会话
inline 读取 artifacts。
...
classify：       AI 按需询问是否创建任务
create：         任务目录 + task.json + 默认 prd.md
plan artifacts： AI 写 prd.md；复杂任务还要写 design.md + implement.md
optional jsonl： AI 按需填写 implement/check spec/research manifest
start：          写入当前 AI 会话/窗口的任务，status=in_progress
implement/check：开发与验证循环
finish：         清除当前 AI 会话/窗口的任务
archive：        把完成任务移动到 archive/
```

`task.py create` 总是创建默认 `prd.md` 模板。检测到 sub-agent 平台（Claude /
Cursor / Codex / Kiro / Pi 等）时，也可能 seed `implement.jsonl` + `check.jsonl`。
它不创建 `design.md` 或 `implement.md`；复杂任务由 AI 在 planning 阶段写这两个文件。
...
`task.py add-context` 只写 `implement.jsonl` / `check.jsonl`。Research 结论应该
以 markdown 文件形式放在 `$TASK_DIR/research/`。

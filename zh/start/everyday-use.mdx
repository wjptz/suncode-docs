...
`continue` 是**单个任务内**的 continue，不是跨任务的 continue。它读当前会话 runtime 文件（`.trellis/.runtime/sessions/<session-key>.json`）和该任务的 `status`，对照 `workflow.md` 判断当前处于哪个 phase/step，然后推进到下一步。

...
| **新开发者加入已有项目** | `trellis init -u their-name` | 生成 joiner 引导 task `00-join-<slug>`，自动设为 current-task，引导读 workflow + spec |
...
<Note>
Agent-capable 平台（Claude Code、OpenCode、Cursor、Gemini、Qoder、CodeBuddy、Copilot、Droid，以及开了 `codex_hooks` 的 Codex）每次开会话 SessionStart hook 自动加载 workflow 和上下文，这些平台本身不发 `/trellis:start` 这个 slash 命令；想重新加载就新开一个会话。只有 Kilo / Antigravity / Windsurf 这类 agent-less 平台会发 `/trellis:start`（或 `/start.md`）作为显式入口。
</Note>

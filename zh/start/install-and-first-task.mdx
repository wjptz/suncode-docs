...
<Note>
Agent-capable 平台（Claude Code、OpenCode、Cursor、Gemini、Qoder、CodeBuddy、Copilot、Droid、Pi Agent，以及开了 `codex_hooks` 的 Codex）每次开会话都会通过 SessionStart hook 或 extension 自动加载 workflow 和上下文，这些平台本身不发 `/trellis:start` 这个 slash 命令；想重新加载就新开一个会话。只有 Kilo / Antigravity / Windsurf 这类 agent-less 平台会发 `/trellis:start`（或 `/start.md`）作为显式入口。
</Note>
...
**AI 停下或跳步骤时**：跑 `/trellis:continue`，AI 读当前会话 runtime（`.trellis/.runtime/sessions/<session-key>.json`）+ task status，按 workflow.md 判断下一步该做什么（before-dev / check / update-spec 等）继续推进。
...
Journal 存在 `.trellis/workspace/{name}/journal-N.md`；每次 `/trellis:finish-work` 都会追加一条。`.trellis/.runtime/sessions/<session-key>.json` 负责把当前 AI 会话/窗口指向未完成任务；`task.py finish` 会清掉它。

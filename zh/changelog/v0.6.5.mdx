...
内置 workflow 现在会把 class-2 implement dispatch（codex-sub-agent、Gemini、Qoder、Copilot、ZCode、Reasonix、Trae）保持在 pull-based block，而不是 hook auto-handles block。这样 workflow 文案与生成出来的 pull-based sub-agent context loading 保持一致。
...
ZCode command fallbacks 现在保留在 `.zcode/commands/trellis/`，不再写入共享的 `.agents/skills/` 目录。这样 Codex + ZCode 组合安装后，`trellis update --dry-run` 不会立刻报告 template drift。

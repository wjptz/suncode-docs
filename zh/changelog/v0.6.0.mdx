...
<Tip>
  **多 agent 协作在 v0.6 升为一等原语。** `trellis channel` 提供 worker-supervisor 运行时（Claude
  `stream-json` + Codex `app-server` adapter）、`~/.trellis/channels/` 下的持久事件日志、forum /
  thread 风格 issue 板、默认 OOM 防护，以及 `@mindfoldhq/trellis-core/channel` 暴露的生命周期 / 订阅
  API。`trellis init` / `trellis update` 现在会把内置的 `check` / `implement` agent
  定义自动下发到每个新装项目，因此 `trellis channel spawn --agent check` 开箱即用。详见下方 "[多
  agent 协作](#多-agent-协作)" 段。
</Tip>

...
Claude Code statusLine 默认不再为新项目安装。新项目不会写 `.claude/hooks/statusline.py`，也不会在 `.claude/settings.json` 里配置 `statusLine`。

已有项目会保留已经安装的 Claude Code statusLine 行为：`trellis update` 会保留 `.claude/hooks/statusline.py`，并把 `.claude/settings.json` 里已有的 `statusLine` 配置带到更新后的 settings 文件里。

manifest 包含 10 条 hash-verified `safe-file-delete` entry，用于清理曾写入项目但现在已经孤立的 hook。

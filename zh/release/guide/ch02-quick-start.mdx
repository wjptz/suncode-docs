...
<Note>**操作系统要求**: Mac、Linux、Windows 完整支持。脚本已全部迁移为 Python，跨平台兼容。</Note>

初始化命令根据你使用的平台选择：

<Tabs>
  <Tab title="Claude Code">
    ```bash trellis init -u your-name ``` 自动检测 Claude Code 环境，配置 `.claude/`
    目录（commands、agents、hooks）。
  </Tab>
  <Tab title="Cursor">
    ```bash trellis init -u your-name ``` 自动检测 Cursor 环境，配置 `.cursor/commands/` 目录。
  </Tab>
  <Tab title="Codex">
    ```bash trellis init --codex -u your-name ``` 配置 `AGENTS.md` 和 `.agents/skills/`。
  </Tab>
  <Tab title="OpenCode">
    ```bash trellis init --opencode -u your-name ``` 配置 `.agents/skills/`。支持 session hook
    自动注入上下文。
  </Tab>
  <Tab title="Kilo">
    ```bash trellis init --kilo -u your-name ``` 配置 `.kilocode/`
    目录（workflows、rules、skills）。
  </Tab>
  <Tab title="Kiro">
    ```bash trellis init --kiro -u your-name ``` 配置 `.kiro/` 目录（steering、hooks、prompts）。
  </Tab>
  <Tab title="CodeBuddy">
    ```bash trellis init --codebuddy -u your-name ``` 配置 `.codebuddy/commands/trellis/`
    目录（嵌套子目录命名空间的 slash commands）。
  </Tab>
</Tabs>
...
| 能力                 |     Claude Code      |     Cursor     |    Codex     | OpenCode |            Kilo            |        Kiro         |         CodeBuddy          |
| -------------------- | :------------------: | :------------: | :----------: | :------: | :------------------------: | :-----------------: | :------------------------: |
| 会话自动注入         |       ✅ Hook        | ❌ 手动 /start | ⚡ AGENTS.md | ✅ Hook  |  ⚡ rules/ + launchConfig  |    ⚡ steering/     | ❌ 手动 /trellis:start     |
...
| 命令数量             |          13          |       12       |      13      |    13    |         workflows          |       prompts       |             12             |
...
交互式选择器会展示[官方 Marketplace](https://github.com/mindfold-ai/Trellis/tree/main/marketplace) 中的所有模板。你也可以选择空白模板从头开始。

...
| **Monorepo 混乱** | 单一规则文件没法覆盖多个包、每个包规范都不一样 | `trellis init` 自动识别 monorepo，为**每个 package 建独立 spec 目录**，各自执行自己的规范 |
...
| 支持平台 | Cursor 专属 | Claude Code 专属 | 按平台 | **12 个平台**（Claude Code、Cursor、OpenCode、Codex、iFlow、Gemini CLI、Kilo、Kiro、GitHub Copilot、Windsurf、Qoder、Factory Droid） |

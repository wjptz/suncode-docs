...
[Qoder](https://qoder.com) 本身支持 Custom Commands（在 Agent 输入框里输入 `/` 调用，存放在 `~/.qoder/commands/` 或 `<项目>/.qoder/commands/`）。但 Trellis 在 Qoder 上的设计选择是**把工作流以 auto-trigger skill 的形式交付**，让用户不用记命令名。Trellis 不往 `.qoder/commands/` 写东西——那个空间留给你自己的 Custom Commands。
...
- `.qoder/skills/trellis-{name}/SKILL.md` — 每个 Trellis 工作流渲染成带 YAML frontmatter 的 skill（平台根据用户意图自动匹配）
- `.qoder/rules/trellis.md` — 常驻规则，把 Trellis prelude 带进每一次对话

Qoder 没有 Trellis 层的 hook 系统，`.qoder/rules/` 承担了 `SessionStart` 的职能。

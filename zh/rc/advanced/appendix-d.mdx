...
<Note>
  JSONL 注入的内容应该是 **spec 文件**（`.trellis/spec/**`）或**任务的 research 产出**（`{TASK_DIR}/research/*.md`）——也就是「AI 动手前要先读的规则与背景资料」。不要往里塞纯代码文件 / 目录（`src/services/foo.ts`、`packages/<pkg>/` 等）——sub-agent 自己有 Read / Grep，需要看代码时会自己去找，把代码塞进 context 只是徒增 token。
</Note>
...
| 文件              | Sub-agent           | 典型内容                                          |
| ----------------- | ------------------- | ------------------------------------------------- |
| `implement.jsonl` | `trellis-implement` | workflow + 相关 spec 索引 + 任务 `research/` 目录 |
| `check.jsonl`     | `trellis-check`     | 质量相关 spec + finish-work / check 命令规范      |

`trellis-research` 把可持久化的发现写进任务的 `research/` 目录。Trellis 不要求单独的默认 research manifest；调研上下文来自当前请求、已有 spec、任务历史，以及显式给出的调研指令。

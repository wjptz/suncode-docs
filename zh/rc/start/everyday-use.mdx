...
| **Command** | `/trellis:continue`    | 手动                                    | 当前任务内推进下一步工作流，用户不用记 workflow（详见 1.2.4 小节） |
...
#### 1.2.1 `trellis upgrade`：升级 CLI package
...
#### 1.2.4 `/trellis:finish-work`：收尾 + 归档
...
`task.py create` 会让任务从 `planning` 开始，创建默认 `prd.md`，并尽力把当前 AI session 指向新任务。它也会在检测到 sub-agent 平台（Claude / Cursor / Codex / Kiro / Pi 等）时 seed 出 `implement.jsonl` + `check.jsonl`；agent-less 平台（Kilo / Antigravity / Windsurf）跳过 seed，改由 Phase 2 的 `trellis-before-dev` skill 加载 spec。

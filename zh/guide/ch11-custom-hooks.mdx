...
<Note>
  ...
  - **`SessionStart` hook** 在 Claude Code、Cursor、OpenCode、Gemini CLI、Qoder、CodeBuddy、Copilot、Droid 上出厂（以及开了 `~/.codex/config.toml` 里 `codex_hooks = true` 的 Codex）。Kiro 的 Agent Hook 是用户自己配的，Trellis 不出厂任何。
  - **`PreToolUse`（sub-agent 上下文注入）** 在 Claude Code、Cursor、OpenCode、CodeBuddy、Droid 出厂。其他 hook 可用平台通过 sub-agent 内部的 pull-based prelude 替代。
...
### 11.2 `settings.json` 配置（Claude Code）

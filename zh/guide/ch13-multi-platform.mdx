...
**Trellis 到 CodeBuddy 的映射**：

| Trellis / Claude Code | CodeBuddy | 说明 |
|----------------------|-----------|------|
| `CLAUDE.md` | `.codebuddy/.rules/*.md` | CodeBuddy 使用规则文件作为项目上下文 |
| `.claude/commands/*.md` | `.codebuddy/commands/trellis/*.md` | 嵌套目录：`/trellis:start`、`/trellis:check` |
| `.claude/agents/*.md` | `.codebuddy/agents/*.md` | 自定义 AI 子代理（Markdown + YAML 前置元数据） |
| `.claude/hooks/` | `settings.json` hooks | CodeBuddy 在 settings 配置中支持 hooks |

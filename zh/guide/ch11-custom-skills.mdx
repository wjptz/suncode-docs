...
Skill 是 Trellis 0.5 的主要扩展点。出厂的大部分工作流（brainstorm、before-dev、check、update-spec、break-loop）都以 auto-trigger skill 的形式交付。本章说明如何添加自己的 skill。

### 11.1 Command vs. Sub-agent vs. Skill
...
外部 skill：想引入社区 skill，把文件夹拉进对应平台目录并 commit。Trellis 目前没出厂自动化外部 skill 安装器。

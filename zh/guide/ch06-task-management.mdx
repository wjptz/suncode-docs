...
<Note>
  `bootstrap` 任务（`trellis init` 自动创建）走 `packages/cli/src/commands/init.ts` 里独立的 schema——字段子集略不同（无 `title` / `scope` / `package` / `branch` / `base_branch` / `worktree_path` / `pr_url`），且 `status` 初始就是 `in_progress`。`trellis update` 做破坏性升级时会建 migration task，又是另一套 schema，带有 `current_phase` / `next_action` 两个额外字段（这两个字段只在 migration 任务里出现）。
</Note>

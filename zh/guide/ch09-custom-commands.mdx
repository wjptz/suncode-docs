...
Trellis 0.5 只出厂三个 slash 命令（`/trellis:start`、`/trellis:finish-work`、`/trellis:continue`）；过去的其他命令已经迁成 auto-trigger skill（见第 12 章）。当你需要一个用户显式调用的入口时，才新增 slash 命令。
...
### 9.4 示例：`/trellis:deploy-check` 命令

...
### 12.1 加入已初始化的 Trellis 项目

项目已经被别人 `trellis init` 过了，你作为新成员加入——此时只需要初始化**开发者身份**，不要再跑 `trellis init`（那会覆盖现有配置）。
...
- 在 `.trellis/workspace/<your-name>/` 下建出你自己的 journal 目录

之后在支持 SessionStart hook 的平台上开新会话，就会自动注入上下文；没 hook 的平台跑 `/trellis:start`（Kiro 上是 `@trellis:start`）。

<Note>
  `init_developer.py` 会先检查 `.trellis/.developer` 是否已存在。若存在则打印当前身份并退出，不会覆盖。想换身份先手动删 `.trellis/.developer` 再跑。
</Note>

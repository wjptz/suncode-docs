...
Trellis 0.5 原生带的 slash 命令留得很少。Agent-capable 平台（Claude Code、OpenCode、Cursor、Codex 等）只发 `finish-work` 和 `continue`——`start` 不用手动跑，SessionStart hook 每次开会话自动加载 workflow 和上下文；agent-less 平台（Kilo、Antigravity、Windsurf）没有 hook，`start` 才会作为 slash 命令发出来。过去的其他命令全部迁成 auto-trigger skill（见[定制 Skill](./custom-skills)）。自己加 slash 命令的时机：有一个明确想让用户显式调用的入口。

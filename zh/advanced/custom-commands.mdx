...
Trellis 0.5 原生带的 slash 命令留得很少。Agent-capable 平台（Claude Code、OpenCode、Cursor、Codex、Pi Agent 等）只发 `finish-work` 和 `continue`。`start` 通常不是用户可见命令，因为启动上下文通过 SessionStart hook、extension、`AGENTS.md` 或 prompt-hook bootstrap 交付。Agent-less 平台（Kilo、Antigravity、Windsurf）没有 hook，`start` 才会作为 slash 命令发出来。过去的其他命令全部迁成 auto-trigger skill（见[定制 Skill](./custom-skills)）。自己加 slash 命令的时机：有一个明确想让用户显式调用的入口。

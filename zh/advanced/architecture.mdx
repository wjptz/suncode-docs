...
平台没有暴露 session identity 时，`task.py start` 会直接退出并提示显式设置 `TRELLIS_CONTEXT_ID`——当前代码不会写全局 `.trellis/.current-task` fallback。（老项目里残留的 `.trellis/.current-task` 文件会被保留，但 active-task 解析已不再读它。）

...
- **`/trellis:continue`** 读当前会话 runtime（`.trellis/.runtime/sessions/<session-key>.json`）+ 任务 `status`，再对照 `workflow.md` 判断当前在哪一步、下一步该做什么。
...
<Tabs>
  <Tab title="Hook / extension 可用（CC / Cursor / OC / Gemini / Qoder / CodeBuddy / Copilot / Droid / Pi）">
    SessionStart hook 或 extension 在 session 启动时自动触发，平台对应的 `session-start.py` / `session-start.js` / Pi extension 不需要手动调用即完成 Trellis 上下文注入。打开终端就直接描述任务；暴露 `/trellis:start` 的平台可以用它显式查看上下文报告。
  </Tab>
  <Tab title="Codex">
...
<Info>
  Sub-agent 自动规范注入依赖平台的 PreToolUse hook 或 extension 等价机制。当前支持：Claude Code、Cursor、OpenCode、CodeBuddy、Droid、Pi Agent。其他平台由主会话自行读 JSONL 并把相关内容传给 sub-agent。
</Info>

`inject-subagent-context`（Claude Code 的 Python、OpenCode 的 JS、Pi 的 extension 代码）是规范注入引擎。主会话 spawn `trellis-*` sub-agent 时，hook 或 extension 拦截调用并按 JSONL 注入精准上下文。
...
│                                                              │
│  读取发生在 hook / extension，不在 sub-agent 里：             │
│                                                              │
│  1. 读取当前会话 runtime                                     │
...
└───────────────────────┬──────────────────────────────────────┘
                        │
                        ▼
在 hook-push 和 extension-backed 平台上，sub-agent 启动时 prompt 里已经带着这些内容了（它本身不读任何文件）。pull-based 平台则在 sub-agent prelude 里放等价的读取指令：

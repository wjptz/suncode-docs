...
- **`SessionStart` hook / extension** 在 Claude Code、Cursor、OpenCode、Gemini CLI、Qoder、CodeBuddy、Copilot、Droid、Pi Agent 上原生启用（以及开了 `~/.codex/config.toml` 里 `codex_hooks = true` 的 Codex）。Kiro 的 Agent Hook 是用户自己配的，Trellis 不原生带任何。

...
Trellis 默认不安装 Claude Code 的 `statusLine`。新项目不会创建 `.claude/hooks/statusline.py`，也不会在 `.claude/settings.json` 里写入 `statusLine`。如果项目需要自定义状态栏，需要用户自己添加本地 Claude Code `statusLine` 命令；Trellis 不管理这个文件。

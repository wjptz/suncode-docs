...
Skill 在全部 22 个配置平台上都可用，位置按平台不同：
...
| Codex       | `.agents/skills/{name}/SKILL.md`（Codex 专用 override 在 `.codex/skills/`）     |
...
| Kimi Code   | `.agents/skills/{name}/SKILL.md`（私有命令/角色 skill 在 `.kimi-code/skills/`） |
| Snow CLI    | `.snow/skills/{name}/SKILL.md`                                                  |

`.agents/skills/{name}/SKILL.md`（[agentskills.io](https://agentskills.io) 跨平台共享层）由 Codex、Gemini CLI、Pi Agent、Kimi Code 和 ZCode 使用中立 placeholder 写入，各平台输出必须字节一致；Amp、Cline、Deep Agents、Firebender、Warp 等 agent 可直接消费。

...
- `task.py create` 现在在写 `status=planning` 的同时也设置当前 session 的 active-task pointer，所以 `[workflow-state:planning]` 从下一轮就开始生效——brainstorm 和 JSONL curation 阶段就能看到，不必等到 `task.py start`。

...
之前 `/trellis:start` 显式注入的那套上下文 —— 当前开发者身份、git 分支和状态、active task、active tasks 列表、`workflow.md` 的 phase 规则、spec 索引、最近 journal 摘要 —— 0.5.0 之后由 SessionStart hook 在新会话开启时自动注入（Claude Code、Cursor、OpenCode、Gemini CLI、Qoder、CodeBuddy、Copilot、Droid、Pi Agent，以及开了 `codex_hooks = true` 的 Codex）。开新会话直接描述任务即可。
...
```bash
npm install -g @mindfoldhq/trellis@rc     # 1. 升级 CLI 本身
trellis update --migrate                  # 2. 把项目同步到 CLI 当前版本
```

`trellis update` 只把项目升到当前 CLI 版本。如果 CLI 自己还是 0.4.0，那项目最多也只能到 0.4.0。先把全局 CLI 升到 `@rc`，再跑 `trellis update --migrate`。

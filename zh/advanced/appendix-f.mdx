...
有 SessionStart hook 或 extension 的平台（Claude Code、Cursor、OpenCode、Gemini CLI、Qoder、CodeBuddy、Copilot、Droid、Pi Agent——以及开了 `codex_hooks = true` 的 Codex）上下文自动注入，直接描述任务就行。没有自动会话注入的平台（Kilo、Antigravity、Windsurf）跑 `/trellis:start` 或对应平台的 start workflow 加载完整上下文报告。

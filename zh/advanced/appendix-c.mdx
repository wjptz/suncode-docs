...
对齐 `.suncode/scripts/common/task_store.py` 里 `task.py create` 当前写出的结构：
...
```json
{
  ...
  } // 创建时的 execution policy 快照；meta.hub 等其他 namespace 可以并存
}
```

字段顺序与 `.suncode/scripts/common/task_store.py` 里 `task_data` 一致——`task.py` 就按这个顺序写入。

<Note>
  `worktree_path` / `commit` / `pr_url` 是 schema 占位字段，创建时写 null 之后没有 Suncode
...
新任务的 `meta.execution` 来自当时的 `execution.dag.enabled` 和
`execution.dag.require_for_complex_tasks`。这个快照避免之后的配置变更倒过来
给已有任务增加 planning gate。没有这个 namespace 的旧任务走兼容路径。
`meta` 仍可扩展，例如 Hub binding data 与 `meta.execution` 并列放在 `meta.hub`。

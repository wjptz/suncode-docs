...
<Note>
  本文写于 Trellis 0.4.x 时代。部分命名已经变化：Ralph Loop、`dispatch` / `plan` / `debug` agent、Multi-Agent Pipeline 在 0.5.0-beta.0 移除。高层的 K8s 类比仍然适用；本文具体的 agent / hook 名字按历史参考看。
</Note>
